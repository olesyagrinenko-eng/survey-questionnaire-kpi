...
KPI_FRAMEWORK_DEFAULT_PATH = "/Users/olesya.starostina/Documents/Cursor/анкеты тесты РК/KPI_framework_ads_FULL.xlsx"
...
def _load_kpi_framework_df() -> pd.DataFrame:
    if not os.path.exists(KPI_FRAMEWORK_DEFAULT_PATH):
        raise FileNotFoundError(f"Файл KPI не найден: {KPI_FRAMEWORK_DEFAULT_PATH}")
    df = pd.read_excel(KPI_FRAMEWORK_DEFAULT_PATH)
...
def _build_questionnaire_docx(
    ...
    criteria = [
        ...
        f"Когда использовать: содержит '{when_filter}'" if when_filter else "Когда использовать: без фильтра",
...
@app.route("/questionnaire-builder", methods=["GET", "POST"])
def questionnaire_builder():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(url_for("questionnaire_builder"))
